...
from movies.models import UserFavorite, UserWatchlist, UserReview
...
from movies.services.services import (
    ...
    get_recommendations as tv_recommendations,
)
...
class MediaViewSet(viewsets.ViewSet):
    ...
    def trending_movies(self, request):
        ...
        return paginate_queryset([m for m in movies if m], request, lambda m: MovieSerializer(m).data)
    ...
    def trending_tv(self, request):
        ...
        return paginate_queryset([s for s in shows if s], request, lambda s: TVShowSerializer(s).data)
    ...
    def add_watchlist(self, request, pk=None):
        ...
        if not created:
            return Response({"detail": "Already in watchlist."}, status=status.HTTP_400_BAD_REQUEST)
